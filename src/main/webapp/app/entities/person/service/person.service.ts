...
type RestOf<T extends IPerson | NewPerson> = Omit<T, 'lastEventDate' | 'firstEventDate'> & {
...
export class PersonService {
  ...
  protected convertDateFromClient<T extends IPerson | NewPerson | PartialUpdatePerson>(person: T): RestOf<T> {
    return {
      ...person,
      lastEventDate: person.lastEventDate?.format(DATE_FORMAT) ?? null,
      firstEventDate: person.firstEventDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPerson: RestPerson): IPerson {
    return {
      ...restPerson,
      lastEventDate: restPerson.lastEventDate ? dayjs(restPerson.lastEventDate) : undefined,
      firstEventDate: restPerson.firstEventDate ? dayjs(restPerson.firstEventDate) : undefined,
    };
  }
